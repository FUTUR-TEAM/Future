...
macro_pie_chart <- function(macro_percent){
  ...
  assertthat::assert_that(all(colnames(macro_percent) == c("macro_name", "sum", "percent")),
                          msg = "macro_percent must have 3 columns: 'macro_name', 'sum', 'percent'")
  assertthat::assert_that(all(macro_percent$macro_name == c("protein", "fat", "carbohydrates")),
...
pie_chart <-
  plotly::plot_ly(
    macro_percent,
    labels = macro_percent$macro_name,
    values = macro_percent$percent,
    type = 'pie',
    textposition = 'inside',
